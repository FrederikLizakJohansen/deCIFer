...
from decifer.generation import fix_generated_cif, load_model_from_checkpoint
...
def active_elements_from_layout(layout: List[List[Optional[str]]]) -> List[str]:
    return [element for row in layout for element in row if element is not None]


def read_experimental_zip(zip_path: str) -> pd.DataFrame:
    ...
    with zipfile.ZipFile(zip_path, 'r') as archive:
        for info in archive.infolist():
            ...
            if not (filename.endswith('.xy') or filename.endswith('.xye')):
                continue
...
def preprocess_experimental_frame(
    df_exp: pd.DataFrame,
    target_file: str,
    ...
    df_sel = df_exp[df_exp['source_file'].str.lower() == target_file.lower()].copy()
    ...
    if background_file is not None:
        bg_df = df_exp[df_exp['source_file'].str.lower() == background_file.lower()].copy()
        if wavelength is not None:
        ...
        if valid.any():
            scale = (df_sel.loc[valid, "intensity"] / df_sel.loc[valid, "background_intensity"]).min()
        else:
            scale = 1.0
        df_sel["scaled_background"] = scale * df_sel["background_intensity"]
...
def space_group_symbols_for_system(crystal_system: str, include: bool = True) -> List[str]:
    symbols: List[str] = []
...
class DeciferPipeline:
    ...
    def __init__(self, model_path: str, zip_path: str, device: str = "cuda",
        ...
        self.periodic_table_layout = PERIODIC_TABLE_LAYOUT
        self.global_inactive_elements_list = active_elements_from_layout(self.periodic_table_layout)
        ...
        self.df_exp: pd.DataFrame = read_experimental_zip(zip_path)
    ...
    def read_experimental_data(self, zip_path: str) -> pd.DataFrame:
        ...
        """
        return read_experimental_zip(zip_path)
    ...
    def standardize_signal(
        ...
        """
        return standardize_signal_frame(df, q_col, intensity_col, q_min=q_min, q_max=q_max, n_points=n_points)
    ...
    def fix_symmetry_in_cif(self, cif_string: str) -> str:
        ...
        """
        return fix_generated_cif(cif_string)
    ...
    def preprocess_generic(
        ...
        """
        return preprocess_experimental_frame(
            self.df_exp,
            target_file=target_file,
            wavelength=wavelength,
            q_min_crop=q_min_crop,
    ...
    def get_space_group_symbols(self, crystal_system: str, include: bool = True) -> List[str]:
        ...
        """
        return space_group_symbols_for_system(crystal_system, include=include)
