...
### Synthetic Artefact Tests (`bin/ablation.py`)

`bin/ablation.py` evaluates deCIFer's robustness by systematically sweeping over PXRD distortion parameters and measuring whether the model can still recover the correct crystal structure. For each parameter value it generates a batch of structures and records R_wp, structure matching (RMSD), peak similarity, and validity.

**Supported distortion parameters** (passed via `params_dict` in the config):

| Parameter | Description |
|---|---|
| `noise` | Gaussian noise added to intensities |
| `particle_size` | Scherrer broadening crystallite size (Å) |
| `base_fwhm` | Baseline peak FWHM |
| `peak_asymmetry` | Asymmetric peak distortion |
| `eta` | Pseudo-Voigt mixing parameter (0 = Gaussian, 1 = Lorentzian) |
| `q_shift` | Rigid shift of the Q-axis |
| `q_post_scale` | Multiplicative scaling of Q after simulation |
| `q_pre_scale_uniform` | Uniform lattice parameter scaling (scalar) |
| `q_pre_scale_abc` | Per-axis lattice scaling (3-element list) |
| `mask_ranges` | List of Q ranges to zero out, e.g. `[[3.0, 5.0], [7.0, 10.0]]` |
| `preferred_orientation_range` | `[min, max]` range for preferred orientation factor |
| `chebychev_norm_coeffs` | List of Chebyshev background coefficient vectors |
| `phase_scales` | Multi-phase mixing (triggers a second CIF) |

**Configuration** — the script is driven by a YAML file. Example configs are provided in `configs-TRW-CSP-PXRD/`:

```yaml
dataset_path: data/noma/serialized/test.h5
model_path: deCIFer_model/ckpt.pt
output: results/ablation_cubic_artefacts.pkl
seed: 42

# Dataset filters (all optional)
crystal_system: cubic          # triclinic/monoclinic/orthorhombic/tetragonal/trigonal/hexagonal/cubic
target_elements: ["Fe", "O"]   # filter by elements present
element_match_mode: exact      # "exact" or "contains"
element_count: 2               # optional: binary, ternary, etc.
...
# Parameters to sweep (each key maps to a list of values; each is swept independently)
params_dict:
  particle_size: [1, 5, 10, 20, 30, 50, 100]
  peak_asymmetry: [0.0, 0.05, 0.1, 0.2, 0.5, 1.0]
  q_pre_scale_uniform: [0.9, 0.95, 0.99, 1.0, 1.01, 1.05, 1.1]

# Default PXRD parameters applied on non-swept axes
default_params_dict:
  base_fwhm: 0.05
```
...
```bash
python bin/ablation.py --config configs-TRW-CSP-PXRD/ablation_config_cluster.yaml
```

Results are saved as a pickle file containing a `results` dict keyed by parameter name and value. Each entry holds a list of experiment results (generated CIF, reference CIF, R_wp, structure RMSD, peak similarity, validity flags) and the single best result by R_wp.

---

### Running on Experimental Data

#### `bin/run_protocol.py`

`bin/run_protocol.py` is the entry point for the CeO₂ nanoparticle experiment from the paper. It reads synchrotron PXRD scans from a ZIP archive and runs deCIFer under several conditioning protocols (unconstrained, space-group-constrained, composition-constrained, and combinations thereof).

```bash
python bin/run_protocol.py \
  --model-path deCIFer_model/ckpt.pt \
  --zip-path experimental_data.zip \
  --n-trials 25 \
  --suffix run1
```
...
- `--zip-path`: Path to the ZIP file containing experimental PXRD scans.
- `--n-trials`: Number of generation attempts per protocol (default: 25).
- `--suffix`: Suffix appended to the output folder name (default: `default`).
- `--debug-max`: Limit processing to the first N scan files (for quick testing).

Output is written to `particles_CeO2_protocol_<suffix>/` as one pickle file per scan × protocol combination.

#### Using `DeciferPipeline` directly

For custom datasets, use the `DeciferPipeline` class from `bin/experimental_pipeline.py`:
...
# Preprocess one scan (background subtraction, Q-conversion, normalization)
pipeline.prepare_target_data(
    target_file="scan-4907_mean.xy",
    background_file="scan-4903_mean.xy",  # optional
    wavelength=None,   # provide wavelength in Å if data is in 2θ; None if already in Q
    q_min_crop=1.5,
    q_max_crop=8.0,
)

# Run generation trials with optional constraints
pipeline.run_experiment_protocol(
    n_trials=25,
    composition="Ce4O8",       # optional: fix composition
    spacegroup="Fm-3m_sg",     # optional: fix space group
    crystal_systems=[7],       # optional: restrict to cubic (7)
    save_to="ceo2_Fm-3m.pkl",
    protocol_name="Ce4O8_Fm-3m",
)
```

`prepare_target_data` handles:
- Conversion from 2θ to Q (if `wavelength` is given).
- Scaled background subtraction.
- Min-max normalization and interpolation onto a common Q grid (0–10 Å⁻¹, 1000 points).

`run_experiment_protocol` saves a pickle containing the generation config, raw and processed experimental signals, and a list of generated CIF strings and parsed structures.
